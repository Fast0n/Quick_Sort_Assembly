...
.text
start:	ADDI r20, r0, 1         ;registro valore 1
	LB r1, sx(r0)		;carica in r1, il valore di sx
	LB r2, dx(r0)		;carica in r2, il valore di dx

loop0:  ADD r6, r0, r1		;assegna ad i il valore di sx
	...
	LB r5, a(r5)		;carica il valore del pivot in r5
        LB r9, a(r6)            ;caricamento valori per loop2 (rimozione stalli)
        LB r21, a(r7)           ;carimanento valori per loop3 (rimozione stalli)

loop1: BEQ r6, r7, loop2	;se la condizione i=j è vera passa a loop2
...
loop2: SLT r10, r9, r5		;controlla se a[i]<pivot il risultato (booleano) viene inserito in r10
	BEQ r10, r0, loop3	;se la condizione r10=0 è vera passa a loop3
	ADDI r6, r6, 1		;incrementa i 
 	LB r9, a(r6)            ;carica in r9 il valore dell'array nella posizione i (rimozione stalli)
        J loop2			;salto incondizionato

loop3:	SLT r11, r5, r21        ;controlla se pivot>a[j] il risultato (booleano) viene inserito in r11
        BNE r11, r20, loop4       ;se la condizione r11=1 è falsa passa a fif
        ADDI r7, r7, -1		;decrementa j
	LB r21, a(r7)		;carica in r9 il valore dell'array nella posizione 'j' (rimozione stalli)
        J loop3			;salto incondizionato

loop4: 	BEQ r6, r7, loop6	;se la condizione i=j è vera passa a loop6
...
loop5:	LB r13, a(r6)		;carica in r13 il valore dell'array nella posizione 'i'
...
loop6:	ADDI r6, r6, 1		;incrementa i
	ADDI r7, r7, -1		;decrementa j 
        J loop1			;salto incondizionato

loop7:	SLT r15, r1, r7		;Controlla se sx<j il risultato booleano viene inserito in r15
	BEQ r15, r20, loop8	;se la condizione r15=1 è vera passa a loop8
        J loop9                 ;salto incondizionato

loop8:	ADD r19, r0, r1         ;backup del valore di sx in r19
        ADD r17, r0, r2         ;backup del valore di dx in r17
        ADD r2, r0, r7         	;sostituisci il valore di dx con j
	J loop0			;salto incondizionato

loop9:	ADD r2, r0, r17         ;sostituisci il valore di r17 con dx
        SLT r16, r6, r17	;controlla se i<dx il risultato (booleano) viene inserito in r16
        BEQ r16, r0, end	;se la condizione r16=0 passa a end
          
loop10:	ADD r1, r0, r6		;sostituisci il valore di 'sx' con 'i'
        J loop0			;salto incondizionato
    
end:	HALT                    ;interruzione programma
